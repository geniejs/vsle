...
export const ImmigratePanel = () => {
  ...
  const allclsx = [...textareaclsx.valid, ...textareaclsx.invalid];

  const handleImmigrationCodeChange = (event: any) => {
    if (event.target instanceof Element) {
      const textarea = event.target;

      allclsx.forEach((cn) => textarea.classList.remove(cn));
      ...
      try {
        ...
        if (
          ...
          textareaclsx.invalid.forEach((cn) => textarea.classList.add(cn));
          return;
        }

        textareaclsx.valid.forEach((cn) => textarea.classList.add(cn));
